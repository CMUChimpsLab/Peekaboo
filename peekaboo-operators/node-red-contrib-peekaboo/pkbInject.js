...
module.exports = function(RED) {
  ...
  function pkbInjectNode(n) {
      ...
      node.props.forEach(function (prop) {
          if (prop.vt === "jsonata") {
              try {
                  ...
                  node.error(RED._("node-red:inject.errors.invalid-expr", {error:err.message}));
                  prop.exp = null;
              }
          }
      });
      ...
      if (node.repeat > 2147483) {
          node.error(RED._("node-red:inject.errors.toolong", this));
          delete node.repeat;
      }
      ...
      node.repeaterSetup = function () {
          if (this.repeat && !isNaN(this.repeat) && this.repeat > 0) {
              ...
              if (RED.settings.verbose) {
                  this.log(RED._("node-red:inject.repeat", this));
              }
              ...
              if (RED.settings.verbose) {
                  this.log(RED._("node-red:inject.crontab", this));
              }
  ...
  RED.nodes.registerType("pkbInject", pkbInjectNode);

  pkbInjectNode.prototype.close = function() {
      if (this.onceTimeout) {
          clearTimeout(this.onceTimeout);
      }
      if (this.interval_id != null) {
          clearInterval(this.interval_id);
          if (RED.settings.verbose) { this.log(RED._("node-red:inject.stopped")); }
  ...
  RED.httpAdmin.post("/inject/:id", RED.auth.needsPermission("node-red:inject.write"), function(req,res) {
