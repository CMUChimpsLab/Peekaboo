...
const models = require("./resources/models/detect.json");
const noderedutil = require("./util-nodered.js");
const performanceLogger = require("./performanceLogger");
const OccupancyDetectionService = require("./inference/occupancy-detection-service")
...
module.exports = function (RED) {
  ...
  const occupancyDetectionService = new OccupancyDetectionService();
  for (let model of models) {
    model.targets.forEach((target) => {
      let handler = externalService;
      if (model.model === "occupancy") {
        handler = occupancyDetectionService;
      }
      registerTarget(model.datatype, target, model.service, handler);
    });
  }

  function PeekabooDetectNode(config) {
    RED.nodes.createNode(this, config);

    var node = this;
    node.dataaction = "detect";
    node.datatype = config.datatype;
    node.tabularField = config.tabularField;
    node.sampleCount = config.sampleCount;
    node.occupancyThreshold = config.occupancyThreshold;
    node.occupancyInterval = config.occupancyInterval;
  ...
  RED.nodes.registerType("detect", PeekabooDetectNode);
